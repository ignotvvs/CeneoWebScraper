...
class Product():
    ...
    def __str__(self) -> str:
        text = f"""product_id: {self.product_id}<br>
    ...
    def export_opinions(self):
        ...
        with open(f"app/opinions/{self.product_id}.json", "w", encoding="UTF-8") as jfk:
            json.dump([opinion.to_dict() for opinion in self.opinions], jfk, indent=4, ensure_ascii=False)

    def export_product(self):
        if not os.path.exists("app/products"):
            os.makedirs("app/products")
        with open(f"app/products/{self.product_id}.json", "w", encoding="UTF-8") as jfk:
            json.dump(self.stats_to_dict(), jfk, indent=4, ensure_ascii=False)   

    def import_product(self):
        if os.path.exists(f"app/products/{self.product_id}.json"):
            with open(f"app/products/{self.product_id}.json", "r", encoding="UTF-8") as jfk:
                product = json.load(jfk)
            ...
            with open(f"app/opinions/{self.product_id}.json", "r", encoding="UTF-8") as jfk:
                opinions = json.load(jfk)
                for opinion in opinions:
